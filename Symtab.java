...
//added 3 functions
//Functions to get name, level, line from Entry
    public ArrayList<String> EntryName() {
	String nameEntry="";
	ArrayList<String> ListName = new ArrayList<String>();

	for( ArrayList<Entry> block: st) {
	    for (Entry p: block) {
		nameEntry=p.RetName();
		ListName.add(nameEntry); //adding each variable in list	
            }
        }
	return ListName;
    }

    public ArrayList<Integer> EntryLine() {
        int lineEntry = 0;
	ArrayList<Integer> ListLine= new ArrayList<Integer>();
        
	for( ArrayList<Entry> block: st) {
            for (Entry p: block) {
                lineEntry=p.RetNumber();
		ListLine.add(lineEntry); //adding respective line numbers for each var into list	
            }
        }
        return ListLine;
    }

    public ArrayList<Integer> EntryLevel() {
        int levelEntry = 0;
	ArrayList<Integer> ListLevel = new ArrayList<Integer>();

        for( ArrayList<Entry> block: st) {
            for (Entry p: block) {
                levelEntry=p.RetLevel();
		ListLevel.add(levelEntry); //adding respective levels of each var in list
            }
        }
        return ListLevel;
    }
